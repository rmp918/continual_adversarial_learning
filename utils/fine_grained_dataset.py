...
def train_loader(path, train_batch_size, num_workers=0, pin_memory=False, normalize=None):
...
def val_loader(path, val_batch_size, num_workers=0, pin_memory=False, normalize=None):
    if normalize is None:
        normalize = transforms.Normalize(
            mean=IMAGENET_MEAN, std=IMAGENET_STD)
    torch.cuda.empty_cache()
...
def train_loader_cropped(path, train_batch_size, num_workers=0, pin_memory=False):
    normalize = transforms.Normalize(
        mean=IMAGENET_MEAN, std=IMAGENET_STD)
...
def val_loader_cropped(path, val_batch_size, num_workers=0, pin_memory=False):
    normalize = transforms.Normalize(
        mean=IMAGENET_MEAN, std=IMAGENET_STD)
