...
def main():
    ...
    if args.arch == 'vgg16_bn_cifar100':
        model = packnet_models1.__dict__[args.arch](pretrained=False, dataset_history=dataset_history, dataset2num_classes=dataset2num_classes)
    elif args.arch == 'resnet18':
        model = packnet_models1.__dict__[args.arch](dataset_history=dataset_history, dataset2num_classes=dataset2num_classes)
    else:
        print('Error!')
        sys.exit(0)
    ...
    attack = LinfPGDAttack(model, k=10)
    #attack_val = FastGradientSignUntargeted(model, epsilon = 0.0157, alpha = 0.00784, min_val=0, max_val=1, max_iters=1, _type='linf')
    attack_val = LinfPGDAttack(model,k=40)

    manager = Manager(args, model, shared_layer_info, masks, train_loader, val_loader,args.adv_train, attack, attack_val)
