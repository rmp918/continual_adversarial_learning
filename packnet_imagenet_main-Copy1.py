F"""Main entry point for doing all stuff."""
import argparse
import json
import warnings
...
from utils.packnet_manager_Copy1 import Manager
...
##adv_train
parser.add_argument('--adv_train', action='store_true', default=True)
...
def main():
    ...
    if args.use_imagenet_pretrained and model.module.datasets.index(args.dataset) == 0:
        ...
        if args.arch == 'vgg16_bn':
            ...
            for name, param in state_dict.items():
                if 'classifier' not in name:
                    curr_model_state_dict['module.' + name].copy_(param)
            curr_model_state_dict['module.features.32.weight'].copy_(state_dict['classifier.0.weight'])
            curr_model_state_dict['module.features.32.bias'].copy_(state_dict['classifier.0.bias'])
            curr_model_state_dict['module.features.35.weight'].copy_(state_dict['classifier.3.weight'])
            curr_model_state_dict['module.features.35.bias'].copy_(state_dict['classifier.3.bias'])
            if args.dataset == 'imagenet':
                curr_model_state_dict['module.classifiers.0.weight'].copy_(state_dict['classifier.6.weight'])
                curr_model_state_dict['module.classifiers.0.bias'].copy_(state_dict['classifier.6.bias'])
    ...
    attack = FastGradientSignUntargeted(model, epsilon = 0.0157, alpha = 0.00784, min_val=0, max_val=1, max_iters=1, _type='linf')

    
    

    manager = Manager(args, model, shared_layer_info, masks, train_loader, val_loader, args.adv_train, attack)
    ...
    named_of_params_to_optimize_via_Adam = []

    for name, param in named_params.items():
        if 'classifiers' in name:
            if '.{}.'.format(model.module.datasets.index(args.dataset)) in name:
                params_to_optimize_via_Adam.append(param)
                named_of_params_to_optimize_via_Adam.append(name)
            continue
        else:
            params_to_optimize_via_Adam.append(param)
            named_of_params_to_optimize_via_Adam.append(name)
    ...
    optimizer_network = optim.Adam(params_to_optimize_via_Adam, lr=lr,
                          weight_decay=0.0)
